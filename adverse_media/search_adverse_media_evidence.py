...
class AISearchTool(BaseTool):
    ...
    def _run(self, name: str) -> str:
        ...
        If you can find at least one article, return the <Date></Date>, <Source></Source>, <Headline></Headline> of that article alongside a short <Summary></Summary> of the article. If no such article can be found, state `Not Found`. If
        multiple articles our found, return them one after another.

        When considering the customer name, consider common alternatives, variations or nicknames for matching.
        """
...
class MediaOutput(BaseModel):
    ...
    Evidence: List[Dict] = Field(..., description="List of articles found with date, source and headline extracted into dict.")
...
research = Task(
    ...
    - 5: High Risk - the article(s) is/are recent, the source is reliable and the nature of the activities are highly suspicious.
    ...
    Make sure to consider the severity of the adverse media in the above determination.
    """,
    expected_output="""Return the decison in the following format:
    <Risk>: 1-5
    <Reason>: A short reasoning for the decision</Reason>
    <Evidence>: List of articles found with date, source and headline extracted into dict.
    """,
